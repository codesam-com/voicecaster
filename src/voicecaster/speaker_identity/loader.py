...
def load_aligned_utterances(work_episode_dir: Path) -> list[dict[str, Any]]:
    return _load_json(work_episode_dir / "04_alignment" / "aligned_utterances.json", list)


def load_aligned_turns(work_episode_dir: Path) -> list[dict[str, Any]]:
    return _load_json(work_episode_dir / "04_alignment" / "aligned_turns.json", list)


def load_aligned_words(work_episode_dir: Path) -> list[dict[str, Any]]:
    return _load_json(work_episode_dir / "04_alignment" / "aligned_words.json", list)


def load_speakers_index(work_episode_dir: Path) -> dict[str, Any]:
    return _load_json(work_episode_dir / "04_alignment" / "speakers_index.json", dict)


def load_alignment_metadata(work_episode_dir: Path) -> dict[str, Any]:
    return _load_json(work_episode_dir / "04_alignment" / "alignment_metadata.json", dict)


def load_speaker_segments(work_episode_dir: Path) -> list[dict[str, Any]]:
    return _load_json(work_episode_dir / "03_diarization" / "speaker_segments.json", list)


def load_speaker_metrics(work_episode_dir: Path) -> dict[str, Any]:
    return _load_json(work_episode_dir / "03_diarization" / "speaker_metrics.json", dict)


def load_transcript_with_speakers(work_episode_dir: Path) -> list[dict[str, Any]]:
    return _load_json(work_episode_dir / "03_diarization" / "transcript_with_speakers.json", list)
